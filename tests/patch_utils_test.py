...
def test_pytree_xarray_kspace_to_image_patch_infer():
    input_tree = [
        DataArray(
            dims=("b", "ch", "h", "w"),
            data=np.tile(np.arange(0, 8, dtype=np.float64), (1, 2, 6, 1)),
        ),
        DataArray(
            dims=("b", "ch", "sp", "len"),
    ...
    assert torch.allclose(
        _transfer_to_device(output, torch.device("cpu")),
        _transfer_to_device(input_tree[0], torch.device("cpu")),
    )
...
def test_pytree_xarray_image_patch_infer():
    ...
    assert torch.allclose(
        _transfer_to_device(output, torch.device("cpu")),
        _transfer_to_device(input_tree, torch.device("cpu")),
    )
